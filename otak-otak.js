...
function validateOldPassword(oldPassword) {
    return oldPassword === getAdminPassword();
}
...
function changeAdminPassword(oldPassword, newPassword, confirmPassword) {
    ...
    if (oldPassword === newPassword) {
        showToast('Password baru harus berbeda dari password lama');
        return false;
    }
...
const defaultProfile = {
    ...
    aboutParagraph1: 'Kami berdedikasi untuk menghadirkan otak-otak berkualitas tinggi dengan cita rasa autentik. Setiap produk dibuat dengan penuh perhatian menggunakan bahan-bahan pilihan dan resep turun-temurun yang telah disempurnakan selama bertahun-tahun.',
...
function getAdminPhoto() {
    return localStorage.getItem('adminProfilePhoto') || null;
}

function saveAdminPhoto(base64) {
...
function loadAdminPhoto() {
    const savedPhoto = getAdminPhoto();
    const img = document.getElementById('adminProfilePhoto');
    const initials = document.getElementById('adminAvatarInitials');
    if (savedPhoto && img && initials) {
        img.src = savedPhoto;
...
function handleAdminPhotoChange(event) {
    ...
    reader.onload = function (e) {
        ...
        // Kompres gambar menggunakan canvas agar tidak terlalu besar di localStorage
        const imgEl = new Image();
        imgEl.onload = function () {
            const canvas = document.createElement('canvas');
            const MAX_SIZE = 300; // px
            const ratio = Math.min(MAX_SIZE / imgEl.width, MAX_SIZE / imgEl.height);
            ...
            // Simpan & tampilkan
            saveAdminPhoto(compressed);
            loadAdminPhoto();
            showToast('Foto profil berhasil diperbarui! 📸');
        };
        imgEl.src = originalBase64;
    };
...
function processPayment() {
    ...
    // Show payment method selection modal (Shopee style)
    showPaymentMethodModal(grandTotal);
}
...
function showPaymentMethodModal(amount) {
    ...
    const amountDisplay = document.getElementById('paymentSummaryAmount');
...
function selectPaymentMethod(method) {
    ...
    switch (method) {
        case 'dana':
            processDanaPayment(grandTotal);
            break;
        case 'bank':
            processBankTransfer(grandTotal);
            break;
        case 'qris':
            processQrisPayment(grandTotal);
            break;
        default:
            showToast('Metode pembayaran tidak valid');
    }
}

// Process DANA payment
function processDanaPayment(amount) {
    const customerName = document.getElementById('customerName').value.trim();
    const danaPhone = '082315750980';
    const note = encodeURIComponent(`Bayar Otak-Otak - ${customerName}`);

    // Shopee style: Try to open app directly first
    const isAndroid = /Android/i.test(navigator.userAgent);
    const intentLink = `intent://transfer?phoneNumber=${danaPhone}&amount=${amount}&note=${note}#Intent;scheme=dana;package=id.dana;end`;
    const danaLink = `dana://transfer?phoneNumber=${danaPhone}&amount=${amount}&note=${note}`;

    const link = isAndroid ? intentLink : danaLink;

    // Attempt automatic redirect
    window.location.href = link;

    // Then show modal as backup/confirmation
    setTimeout(() => {
        showDanaModal(amount);
    }, 500);
}

// Process QRIS payment
function processQrisPayment(amount) {
    showQrisModal(amount);
}

// Process Bank Transfer
function processBankTransfer(amount) {
    const customerName = document.getElementById('customerName').value.trim();
    const accountNumber = '1234567890'; // Nomor rekening tujuan
    const note = encodeURIComponent(`Pembayaran ${customerName}`);

    // Try different mobile banking deep link formats
    // BCA Mobile format
    const bcaLink = `bcamobile://transfer?accountNumber=${accountNumber}&amount=${amount}&note=${note}`;

    // Generic banking link (some apps support this)
    const bankLink = `banktransfer://transfer?account=${accountNumber}&amount=${amount}&note=${note}`;

    // Try to open mobile banking app directly
    console.log('Attempting to open mobile banking with:', bcaLink);
    window.location.href = bcaLink;

    // Try alternative format after short delay
    setTimeout(() => {
        window.location.href = bankLink;
    }, 300);

    // Show bank transfer modal as backup (in case app doesn't open)
    // This gives user manual transfer option
    setTimeout(() => {
...
function showBankTransferModal(amount) {
    ...
    const amountDisplay = document.getElementById('bankTransferAmount');
...
// Hide Bank Transfer Modal
function hideBankTransferModal() {
    const modal = document.getElementById('bankTransferModal');
    modal.classList.remove('show');
}
...
function confirmBankTransfer() {
    ...
    const description = `${customerName} - ${dateStr} pukul ${timeStr} - ${orderedItems} - TRANSFER BANK`;
    ...
    showToast(`Pesanan berhasil! Total: ${formatCurrency(grandTotal)} - Silakan transfer ke rekening BCA yang tertera`);
...
// ===================================
// DANA & QRIS PAYMENT MODAL
// ===================================

function showDanaModal(amount) {
    ...
    const amountDisplay = document.getElementById('danaAmountDisplay');
    if (amountDisplay) {
        amountDisplay.textContent = formatCurrency(amount);
    }
    ...
    if (paymentLink) {
        const isAndroid = /Android/i.test(navigator.userAgent);
        paymentLink.href = isAndroid ? intentLink : danaLink;
    }
...
function showQrisModal(amount) {
    ...
    // Update displayed amount
    const amountDisplay = document.getElementById('qrisAmountDisplay');
    if (amountDisplay) {
        amountDisplay.textContent = formatCurrency(amount);
    }

    // Generate QR Code
    generateQrisCode('082315750980'); // Using phone number as source for QR
...
function generateQrisCode(data) {
    ...
    if (typeof QRCode !== 'undefined') {
        ...
        container.innerHTML = '<p>Gagal memuat QR Code</p>';
    }
}

function confirmDanaPayment() {
    confirmPayment('DANA');
}
...
function confirmPayment(method) {
    ...
    const description = `${customerName} - ${dateStr} pukul ${timeStr} - ${orderedItems} - ${method}`;
...
function updateProfileDisplay() {
    ...
    if (instagramLink) {
        instagramLink.href = profile.instagramUrl || 'https://instagram.com/otakotak_umkm';
        const instagramText = instagramLink.querySelector('.contact-text');
        if (instagramText) {
            instagramText.textContent = profile.instagram || '@otakotak_umkm';
        }
    }

    if (facebookLink) {
        facebookLink.href = profile.facebookUrl || 'https://facebook.com/otakotakumkm';
        const facebookText = facebookLink.querySelector('.contact-text');
        if (facebookText) {
            facebookText.textContent = profile.facebookName || 'Otak-Otak';
        }
    }
}
...
function loadProfileForm() {
    ...
    document.getElementById('instagramUrl').value = profile.instagramUrl || 'https://instagram.com/otakotak_umkm';
    document.getElementById('facebookName').value = profile.facebookName || 'Otak-Otak';
    document.getElementById('facebookUrl').value = profile.facebookUrl || 'https://facebook.com/otakotakumkm';
}
...
function switchAdminView(viewName) {
    ...
    if (viewName === 'dashboard') {
        ...
        // Clear password form when switching to this view
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('changePasswordForm').addEventListener('submit', (e) => {
        ...
        if (changeAdminPassword(oldPassword, newPassword, confirmPassword)) {
            document.getElementById('changePasswordForm').reset();
        }
    });

    // Mobile back to order button
    const mobileBackBtn = document.getElementById('mobileBackToOrder');
    if (mobileBackBtn) {
    ...
    // Load admin profile photo dari localStorage
    loadAdminPhoto();
});
...
function simpanKeSheet() {
    ...
    fetch("https://script.google.com/macros/s/AKfycbxNvGwFj1iNbS3S98NcRNn7EPHjTRFLV0f-DZaORyi94dnN7Vg1zz0fEdgW-xc6EKgjNw/exec", {
        method: "POST",
        mode: "no-cors",
        headers: {
        ...
        .then(() => {
            console.log("Data berhasil dikirim ke Sheet");
        })
        .catch((error) => {
