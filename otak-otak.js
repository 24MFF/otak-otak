...
function processPayment() {
    ...
    const merchantName = 'Otak-Otak';
    const danaLink = `dana://qr?amount=${grandTotal}&note=Pembayaran ${merchantName}&to=${danaPhoneNumber}`;

    // Try to open DANA app directly
    window.location.href = danaLink;

    // Show QRIS modal as backup (in case DANA app doesn't open)
    setTimeout(() => {
...
function showQRISModal(amount) {
    console.log('=== showQRISModal called ===');
    ...
    if (!modal) {
        console.error('ERROR: qrisModal element not found!');
        alert('Error: Modal tidak ditemukan! Periksa console untuk detail.');
        return;
    }
    ...
    if (amountDisplay) {
        ...
        console.log('Amount updated to:', formatCurrency(amount));
    }
    ...
    if (paymentLink) {
        ...
        console.log('DANA link set:', danaLink);
    }

    // Try to generate QR Code if library is available
    if (typeof QRCode !== 'undefined') {
        console.log('QRCode library available, generating QR...');
    ...
    console.log('=== showQRISModal complete ===');
}
...
function generateQRCode(amount) {
    ...
    if (!qrisContainer) {
        console.error('qrisCode container not found');
        return;
    }
    ...
    if (typeof QRCode === 'undefined') {
        ...
        qrisContainer.innerHTML = `
            <div class="qr-placeholder">
                <p>⚠️ QR Code library tidak tersedia</p>
                <p style="font-size: 12px; color: #666;">Gunakan Cara 1 di atas atau transfer manual ke nomor DANA di bawah</p>
            </div>
        `;
        return;
    }
    ...
    try {
        QRCode.toCanvas(canvas, qrData, {
            ...
            if (error) {
                ...
                qrisContainer.innerHTML = `
                    <div class="qr-placeholder">
                        <p>⚠️ Gagal membuat QR Code</p>
                        <p style="font-size: 12px; color: #666;">Error: ${error.message}</p>
                        <p style="font-size: 12px; color: #666;">Gunakan Cara 1 atau transfer manual</p>
                    </div>
                `;
            } else {
        ...
        qrisContainer.innerHTML = `
            <div class="qr-placeholder">
                <p>⚠️ Error saat membuat QR Code</p>
                <p style="font-size: 12px; color: #666;">Gunakan transfer manual ke nomor di bawah</p>
            </div>
        `;
    }
}
